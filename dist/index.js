...
function setCors(app) {
	const whitelist = ['http://127.0.0.1:3000', 'http://localhost:3000', 'https://nusic.vip', 'https://www.nusic.vip'];
...
app.post(apiBaseUrl + '/getTopReferers', function (req, res) {
	...
	try {
		connection.query(sql, function (error, data, fields) {
			if (error) res.send({ success: false, message: error.message });else {
				if (data.length == 0) res.send({ success: false, message: 'no data' });else {
					let list = [];
	...
	return new Promise((resolve, reject) => {
		try {
			connection.query(sql, function (err, result) {
				if (err) return reject(err);else if (result.length === 0) resolve([]);else {
					let referers = [];
					let funded = [];
...
app.post(apiBaseUrl + '/setMetadata', function (req, res) {
